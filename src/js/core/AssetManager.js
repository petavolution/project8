...
window.AssetManager = AssetManager;
    ...
    async loadTextures(textureList, progressCallback) {
        if (!textureList || textureList.length === 0) return Promise.resolve();
        console.log(`Loading ${textureList.length} textures`);
            ...
            return new Promise(resolve => {
                textureLoader.load(
                    ...
                    texture => {
                        ...
                        loaded++;
                        if (progressCallback) {
                            progressCallback(loaded / textureList.length);
                        }
                        resolve(texture);
                    },
                    // On progress
    ...
    async loadModels(modelList, progressCallback) {
        if (!modelList || modelList.length === 0) return Promise.resolve();
        console.log(`Loading ${modelList.length} models`);
        
        // Use the appropriate GLTFLoader
        const loader = new THREE.GLTFLoader();
        let loaded = 0;
        
        // Set up Draco loader if needed
            ...
            return new Promise(resolve => {
                loader.load(
                    ...
                    gltf => {
                        ...
                        loaded++;
                        if (progressCallback) {
                            progressCallback(loaded / modelList.length);
                        }
                        resolve(gltf);
                    },
                    // On progress
     ...
     * @returns {THREE.Texture} The loaded texture or null if not found
     */
    getTexture(key) {
     ...
     * @returns {Object} The loaded GLTF object or null if not found
     */
    getModel(key) {
